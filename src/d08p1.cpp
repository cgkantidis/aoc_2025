...
tests();
u64
get_mul_largest_circuits(std::vector<std::string> const &lines,
                         u64 num_connections,
                         u64 num_largest_circuits);
} // namespace
...
main(int argc, char const **argv) {
  ...
  std::println("{}", get_mul_largest_circuits(lines, 1000, 3));
  return 0;
}
...
tests() {
  ...
  ASSERT(get_mul_largest_circuits(lines, 10, 3) == 40);
}
...
get_mul_largest_circuits(std::vector<std::string> const &lines,
  ...
  for (auto const &dist : distances | std::views::take(num_connections)) {
  ...
  std::ranges::sort(circuits, [](Circuit const &l, Circuit const &r) {
    return l.jbox_indices.size() > r.jbox_indices.size();
  });

  // multiply the largest curcuits
  return std::ranges::fold_left(circuits | std::views::take(num_largest_circuits),
                                1,
                                [](u64 ret, Circuit const &circ) {
                                  return ret * circ.jbox_indices.size();
                                });
}
} // namespace
