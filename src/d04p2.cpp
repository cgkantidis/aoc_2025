...
u64
get_total_paper_rolls_removed(std::vector<std::string> const &lines);
} // namespace
...
main(int argc, char const **argv) {
  ...
  std::println("{}", get_total_paper_rolls_removed(lines));
  return 0;
}
...
tests() {
  std::vector<std::string> const lines{"..@@.@@@@.",
                                       ...
                                       ".@@@@@@@@.",
                                       "@.@.@@@.@."};
  ASSERT(get_total_paper_rolls_removed(lines) == 43);
}
...
get_total_paper_rolls_removed(std::vector<std::string> const &lines) {
  ...
  u64 total_paper_rolls_removed{};
  while (true) {
    Matrix<char> tmp_grid = grid;
    u64 paper_rolls_removed{};
    ...
    if (paper_rolls_removed == 0) {
      break;
    }
    total_paper_rolls_removed += paper_rolls_removed;
    grid = std::move(tmp_grid);
  }
